...
from app.models import HabitacionesDB
...
class TipoHabitacionRepository:
    ...
    def get_by_id(self, id_tipo: int) -> HabitacionesDB:
        return self.db.query(HabitacionesDB).filter(HabitacionesDB.id_tipo == id_tipo).first()
    ...
    def update_tipo(self, id_tipo: int, tipo_data: dict) -> HabitacionesDB:
    ...
    def delete_tipo(self, id_tipo: int) -> bool:
        tipo = self.get_by_id(id_tipo)
        reservas = self.db.query(ReservasDB).filter(ReservasDB.id_tipoHabitacion == id_tipo).count()
        if reservas > 0:
            return False
        self.db.delete(tipo)
        self.db.commit()
